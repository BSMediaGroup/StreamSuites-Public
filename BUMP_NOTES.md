...
- `/games` market/exchange modal fix: Runtime market-exchange catalog rows now include definition `tags`, and Public enriches storefront catalog items before opening the lightbox so Market and Exchange More Details modals show the same hashtag chips as profile inventory.
