...
- Propagated the selected profile theme onto the document element that owns Chromium's outer page scrollbar. The main right-edge thumb now follows every profile preset instead of falling back to the default violet while the body, profile shell, and editor use another theme; internal editor scrollbar behavior is unchanged.
...
- Validation passed JavaScript syntax and all 177 Public Node tests. Deterministic Chromium checks at 1060×640 and 390×844 confirmed the sidebar navigation/active state, absent header chips, defined-link cards, unused-platform menu, repeatable custom card with icon controls, fallback globe visibility, and zero document/modal horizontal overflow. A final computed-style check changed the document-owned outer scrollbar from Signal Red to Emerald Cyan through the real theme control and confirmed that cancelling restored Signal Red. The only console error was the live page's unrelated Cloudflare Insights integrity mismatch; the existing iframe `allowfullscreen` precedence warning also remained.
...
- Added a bounded session-only public-profile cache: responses remain fresh for 60 seconds and may be retained for up to ten minutes only as a transient transport/server-error fallback. Authoritative not-found/private responses are never replaced by stale cache data, owner-only `/profile/me` payloads are not persisted, and Runtime/Auth remains the source of truth.
- Expanded the Pages Function bootstrap to retain the known public-only identity, media, About, social, live, progression, economy, inventory, badge, and theme projection. Missing media now stays absent instead of becoming the SEO share image, and custom hero covers paint above the established cover fallback so a failed custom asset does not leave an empty media frame.
- Routed page-visit analytics through the existing same-origin `/api/public/*` Pages proxy, removing the browser CORS failure without changing analytics authority. Cloudflare Insights, Rumble advertising, and Google IMA requests blocked by a client extension remain outside Public application control; provider Permissions-Policy warnings are not profile hydration failures.
- Validation passed JavaScript/Pages Function syntax, all 173 Public Node tests, and `git diff --check`. Wrangler 4.60.0 compiled the Pages Worker, parsed the existing 20 redirect and one header rules, and served the real `/u/bsmediagroup` Function route with HTTP 200, the new script revision, a 56-field public bootstrap, custom avatar/cover media, and the intended `public, max-age=60, s-maxage=120` HTML policy. Chromium measured zero full profile-shell replacements after first paint, retained the custom CDN avatar/cover and zero horizontal overflow, then repeated that result with the SSR bootstrap deliberately removed and the local client API unavailable; the v2 session cache kept the exact public profile instead of generic defaults. Local-only `127.0.0.1:18087` connection failures and the existing cross-origin Status diagnostics warning were expected; the former analytics CORS error was absent. No deployment, version/build change, commit, push, Runtime/Auth mutation, or provider fetch was performed.
