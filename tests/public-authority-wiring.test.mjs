...
test("games economy page uses gallery-first market groups and shell toolbar anchors", () => {
  ...
  assert.match(app, /buildMarketPriceDisplay\(item, options, \{ className: "market-item-price market-item-price--gallery" \}\)/);
  ...
  assert.match(app, /economyItemCategoryLabel\(item\)/);
  assert.match(app, /economyItemRarityLabel\(item\)/);
  assert.match(app, /economyItemDetailsText\(item\)/);
  assert.match(app, /addMeta\("Item code", item\.item_code \|\| item\.asset_code\)/);
...
test("public economy rendering keeps denominations separate from inventory rows", () => {
  ...
  assert.match(app, /const width = Math\.min\(400, Math\.max\(280, window\.innerWidth - 32\)\)/);
...
test("public economy item tooltip controller keeps one active popover and preserves fallback copy", () => {
  ...
  assert.match(css, /\.item-info-popover\s*\{[\s\S]*grid-template-columns:\s*80px minmax\(0, 1fr\)/);
  assert.match(css, /\.item-info-popover-media\s*\{[\s\S]*width:\s*80px/);
  assert.match(css, /\.item-info-popover-icon\s*\{[\s\S]*width:\s*70px/);
