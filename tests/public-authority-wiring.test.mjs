...
test("public leaderboards route hydrates from authoritative progression API", () => {
  ...
  assert.match(app, /identity\.public_slug[\s\S]*identity\.canonical_user_code[\s\S]*identity\.account_user_code[\s\S]*identity\.user_code/);
  ...
  assert.match(app, /state\.expandedId = isExpanded \? "" : String/);
  ...
  assert.match(app, /addStat\("Level", level\.label/);
  ...
  assert.match(css, /\.progression-rank-position-asset/);
...
test("public profile game section renders runtime progression and economy authority summaries", () => {
  ...
  assert.match(profileSection, /label: "Global rank"/);
  assert.match(profileSection, /buildProgressionGlobalRankValue\(progression, \{ prominent: true, emptyLabel: "Unranked" \}\)/);
  assert.match(profileSection, /Global XP leaderboard placement, separate from/);
