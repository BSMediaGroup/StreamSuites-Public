...
test("public leaderboards route hydrates from authoritative progression API", () => {
  ...
  assert.match(app, /create\("span", "progression-leaderboard-chevron", isExpanded \? "Hide" : "Show"\)/);
  ...
  assert.match(app, /stats\.walletCount \? formatCompactNumber\(stats\.walletTotal\) : "Not public"/);
  ...
  assert.match(app, /create\("span", "ss-lb-sparkle"\)/);
  ...
  assert.match(css, /\.progression-leaderboard-chevron::before/);
  ...
  assert.match(css, /\.progression-leaderboard-profile-link\s*\{[\s\S]*width:\s*min\(100%, 190px\)/);
  ...
  assert.match(css, /\.ss-lb-sparkle/);
  assert.match(css, /@keyframes ss-lb-sparkle-drift/);
  assert.match(css, /prefers-reduced-motion:\s*reduce[\s\S]*\.ss-lb-sparkle::before/);
