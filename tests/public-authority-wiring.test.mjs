...
test("public leaderboards route hydrates from authoritative progression API", () => {
  ...
  assert.match(app, /filteredRows\.forEach\(\(entry\) => list\.appendChild\(buildLeaderboardRow\(entry, state\)\)\)/);
  assert.doesNotMatch(app, /progression-leaderboard-podium/);
  assert.doesNotMatch(app, /topRows\.forEach/);
  ...
  assert.match(css, /\.progression-leaderboard-detail\s*\{[\s\S]*grid-template-columns:\s*minmax\(170px, 230px\) repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(css, /\.progression-leaderboard-detail-stats\s*\{[\s\S]*display:\s*contents/);
  assert.match(css, /\.progression-leaderboard-profile-link\s*\{[\s\S]*width:\s*100%/);
  assert.doesNotMatch(css, /\.progression-leaderboard-podium/);
