...
test("public wheels route preserves the shell and provides clean list/detail artifact routes", () => {
  ...
  assert.match(app, /const wheelSet = item\?\.wheelSet/);
  assert.match(app, /const activeWheel = item\?\.activeWheel/);
  assert.match(app, /title: activeWheel\?\.name \|\| item\?\.title \|\| "Wheel"/);
  assert.match(app, /artifactTitle: artifactItem\?\.title/);
  assert.match(app, /function aggregateWheelEntrants\(entries\)/);
  assert.match(app, /Number\(entry\?\.weight \?\? entry\?\.unitWeight \?\? entry\?\.unit_weight\)/);
  ...
  assert.match(app, /wheelId: String\(activeWheel\?\.wheelId/);
  assert.match(app, /wheel_set: item\?\.wheelSet/);
  assert.match(app, /active_wheel_id: draft\.active_wheel_id/);
  assert.match(app, /data-entry-field="entries"/);
  assert.match(app, /data-entry-field="enabled"/);
  assert.doesNotMatch(app, /class="[^"]*wheel-(?:grid|deck|focus)[^"]*"/);
  assert.doesNotMatch(app, /data-wheel-spin-all/);
