...
- The specialized StudioApp, OBS, and Extensions pages retain their product-specific layouts, real manifest/catalog behavior, fail-closed gates, icons, and route handling. The new shared standalone polish explicitly excludes `.download-surface`, so their manifest clients, HMAC/cookie boundary, controlled redirect, extension schema, and product presentation remain untouched.
