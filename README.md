...
- The wheel detail route now implements the approved `wheelpocv3` direction inside the unchanged Public shell: a cinematic but bounded wheel arena, premium chassis/pointer/hub treatment, radial V2/V3 label orientation, an always-visible primary spin dock, accessible Play/Entrants/Details/Share console tabs, compact owner/authority drawers, and a modal winner celebration. Repeated records are aggregated only for the public presentation and local weighted draw, so each unique entrant appears once with combined entry weight and projected chance; the authoritative payload, owner-only policy, editor save contract, share routes, sound configuration, and local-only result boundary remain unchanged. Reduced motion disables idle drift and non-essential celebration motion, and forced-colour fallbacks preserve controls and content.
