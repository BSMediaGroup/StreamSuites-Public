...
- The Status Center now presents Runtime’s canonical `overall-availability-v1` critical-path history as an explicitly watchdog-observed 5H/24H/7D/30D feature graph, including coverage, downtime, true unknown time, pre-monitoring time, and the Runtime-derived state rail without treating it as official Atlassian uptime. One reusable accessible report modal produces complete or component-scoped `streamsuites-status-report-v1` exports from a presentation-neutral allowlisted model: dependency-free Canvas PNG pages, vector-oriented HTML/SVG print documents for Save as PDF, and formatted JSON downloads. Every format keeps official and direct evidence separate, preserves deferred/provider-managed explanations, includes provenance, and remains available when either optional source is missing; older diagnostics receive truthful unavailable states rather than browser-synthesized overall or 5H history.
