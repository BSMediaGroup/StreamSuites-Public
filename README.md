...
- The wheel detail route now implements the approved `wheelpocv3` direction inside the unchanged Public shell: a cinematic but bounded wheel arena without a redundant outer panel, a compact in-arena title/description header, premium chassis/pointer/hub treatment, radial V2/V3 label orientation, an always-visible primary spin dock, accessible Play/Entrants/Details/Share console tabs, compact owner/authority drawers, and a focus-managed winner reveal with persistent canvas confetti plus setting-controlled fireworks. Repeated records are aggregated only for the public presentation and local weighted draw, so each unique entrant appears once with combined entry weight and projected chance; the authoritative payload, owner-only policy, editor save contract, share routes, sound configuration, and local-only result boundary remain unchanged. Reduced motion and the master animation setting disable non-essential celebration motion, and forced-colour fallbacks preserve controls and content.
...
- `/stats` is a real aggregate view backed by `GET /api/public/stats`. It renders bounded account/profile/artifact totals, active role counts, and UTC monthly account/artifact creation series supplied by Runtime/Auth, reports freshness and coverage explicitly, and shows Unavailable rather than substituting sample numbers when the contract cannot be loaded. The established Total Site Visits, Runtime Event Breakdown, Total Users, Active Installations, Runtime Events, Performance Analytics, and Milestones & Bragging Rights scaffold groups remain visible; only active accounts and account roles hydrate today, while unsupported visits/installations/event/milestone cells say exactly that no public aggregate is published. Its compact hero, header, responsive navigation, first-party lockup, and full footer use the same newer standalone chrome as `/status` and `/health`; the independent shell appearance control remains in the Stats hero. Graph entry motion is finite and disabled by reduced-motion preferences.
