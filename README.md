...
- `css/studio-first-landing.css` owns the cinematic landing layout, responsive product preview, architecture diagram, connected-capability and ecosystem sections, Alpha messaging, footer, and consolidated production auth/access-modal presentation. The modal remains centered at desktop and mobile sizes, retains the complete OAuth, email/password, legal, account-switch, and expandable other-surface controls, and uses the original key icon for temporary bypass access.
