...
- `css/studio-first-landing.css` owns the approved `pocv9` production adaptation: cinematic but bounded hero atmosphere, a non-layout header progress signal, four equal-footprint Browser Studio/StudioApp/Studio for OBS/Public Shell preview states, subtle directional primary-button gradients and local card glow, and the five-node Runtime/Auth authority topology. The preview uses repository-owned media and icon assets, keeps Browser Studio and StudioApp within one shared stage composition, presents OBS as an authorized bridge into an OBS-owned media pipeline, and presents Public as a downstream artifact shell without enlarging it into the suite's media authority. Production `index.html` uses a bounded presentation revision on that stylesheet URL so a newly deployed document cannot reuse an earlier long-cached landing stylesheet. Motion is visibility-aware and has complete reduced-motion fallbacks. The landing header and footer retain the first-party square `assets/logos/ssmainlogosq.webp` plus `assets/logos/wmnew.webp` lockup; the Creator Login destinations, dismissible access notice, auth modal, OAuth/email/password, Turnstile, legal, account-switch, and temporary bypass controls remain unchanged.
...
- Responsive layouts are explicitly defined for desktop, tablet, and narrow mobile widths. A stored expanded desktop sidebar preference is presented as the compact icon rail on narrow viewports without overwriting the user's desktop preference. Skip links, visible focus states, semantic tabs, keyboard arrow navigation, Escape handling, reduced-motion fallbacks, bounded content widths, readable long-form line lengths, and non-destructive card/route motion are part of the shared treatment. The accepted Studio-first production landing remains on its independent `studio-first-landing.css` cascade and is unchanged by this milestone.
