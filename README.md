...
- `/status` is the canonical branded service-health view. Atlassian Statuspage public reads remain its official state/incident source and the floating widget's only service-health source; `/status` may additionally read the sanitized Runtime/Auth public watchdog projection as clearly secondary diagnostics. Runtime/Auth status synchronization and Statuspage mutation authority are unchanged.
