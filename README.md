...
- `js/studio-first-landing.js` owns keyboard-accessible four-state product tabs, the default-on 5.6-second product loop, its four-dot switcher and play/pause control, gentle preview crossfades, ecosystem preview selection, the dismissible Alpha notice, mobile navigation, sticky-header progress, bounded hero particles, fine-pointer parallax/glow, visibility-aware authority-route sequencing, and progressive-enhancement reveals with reduced-motion fallbacks. The loop pauses while the page or hero is not visible and is disabled when reduced motion is requested. It does not own auth, status, version, access enforcement, canonical state, or any product media path.
...
- Responsive layouts are explicitly defined for desktop, tablet, short landscape, and narrow mobile widths. A stored expanded desktop sidebar preference is presented as the compact icon rail on narrow viewports without overwriting the user's desktop preference. Skip links, visible focus states, semantic tabs, keyboard arrow navigation, Escape handling, reduced-motion fallbacks, bounded content widths, readable long-form line lengths, and non-destructive card/route motion are part of the shared treatment. Under reduced motion, auroras, particles, gradient drift, autoplay, preview float/glints, floating signals, and topology animation stop while the complete static atmosphere, output, topology, controls, and 1px page edges remain visible. In normal motion, the canvas remains singular, capped at DPR 1.5 and bounded to 42 mobile or 72–118 desktop particles plus 30/96 links, and pauses while offscreen or document-hidden; inactive preview animation is paused. The central Runtime/Auth card, ports, icon, and orbit now ease to the currently routed Browser, StudioApp, OBS, or Public color as each corresponding topology pulse feeds it.
...
The follow-up corrective MCP evidence is retained under `output/playwright/` as `landing-corrective-browser-final-mcp.png`, `landing-corrective-studioapp-final-mcp.png`, `landing-corrective-architecture-final-mcp.png`, and `landing-corrective-mobile-final-mcp.png`. The same browser session verified the exact 8% POC stage alignment, default cycle then persistent pause, all four Runtime/Auth route colors, zero narrow-viewport overflow, reduced-motion autoplay shutdown, and violet header/footer edge paint before, during, and after deep scrolling. These generated screenshots are validation artifacts, not production imports.
