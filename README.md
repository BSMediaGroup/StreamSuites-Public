...
- Corrective `/stats` graph evidence is retained under `output/playwright/stats-graph-corrective-20260818/`, including the reproduced production failure and local installed-Edge desktop, mobile, and reduced-motion checks against the current live read-only response. These captures validate local source and are not deployment proof.
