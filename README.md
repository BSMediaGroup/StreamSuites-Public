...
- Canonical public profiles resolve at `/u/<slug>`, backed by the authoritative public slug model exported by `StreamSuites`. The standalone route uses a theme-complete adaptive presentation: a fixed glass header that strengthens on scroll, a pinned cover that fades beneath the identity card, a compact hero `Edit profile` affordance, and the ordered optional About, Public Overview, Identity, Watch, Clips, Artifacts, Platform Presence, Share Profile, and Safety sections. Watch retains the latest stream plus a real secondary-source tray while preserving the canonical `#media` and legacy stream anchors; Artifacts owns non-clip published work, progression/inventory, item details, and scoped boards; Clips separates channel-owned work from clips created by the profile using authoritative provenance. Collapsible sections are keyboard-operable, inert while closed, and reachable through canonical and legacy anchors. Theme tokens cover accents, muted surfaces, focus, dividers, rails, buttons, scrollbars, editor chrome, nested panels, and modals without becoming authority. Stable image media uses the direct query-free Runtime/Auth version URL under the strict same-origin `/profile-media/u/...` Pages transport; exact legacy CDN/API version paths normalize once to that canonical URL and are never retried through alternate origins. The transport accepts only exact immutable paths, strips browser credentials/fingerprint headers, validates Runtime's WebP signature before returning the stream, and rejects query strings, HTML/challenge bodies, unsupported types, private/arbitrary hosts, data/blob values, and traversal. One genuine image failure produces the designed fallback; an owner-only replacement diagnostic becomes available without creating another fetch loop.
- The non-collapsible Public Overview is one continuous semantic definition-list surface with emphasized identity/progression, activity, and account-context facts; it does not nest metric cards. The shared navigation registry follows the exact document order, and its active/hover states use the selected profile theme. About, Watch, Artifacts, Clips, Identity, and Safety use title-only feature headers without redundant eyebrow copy. About presents video before story text when both exist and uses a restrained provider attribution link; Watch is initially expanded only when authoritative current, latest, or secondary content exists, while Artifacts starts collapsed. Primary actions and the header mark use the same two theme colours at opposing rest/hover gradient directions rather than a multi-stop highlight spectrum.
